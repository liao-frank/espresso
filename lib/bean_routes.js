...
class BeanRoutes {
	...
	constructor(controllers) {
		this.controllers = __mapNames(controllers);
		this.controller_actions = __mapActions(controllers);
		this.routes = {
			'get': [],
			'put': [],
			'post': [],
		...
		Object.keys(this.routes).forEach((method) => {
			...
			routes.forEach((route) => {
				...
				if (this.controller_actions[controller_name].includes(action_name)) {
					__initRoute(app, method, path, controller_name, action_name, controller.prototype[action_name]);
				}
			});
		});
	}

	__initRoute(app, method, path, controller_name, action_name, action_definition) {
		switch(method) {
			case 'get':
				app.get(path, __extendRequestCallback(controller_name, action_name, action_definition));
			case 'put':
				app.put(path, __extendRequestCallback(controller_name, action_name, action_definition));
			case 'post':
				app.post(path, __extendRequestCallback(controller_name, action_name, action_definition));
			case 'delete':
				app.delete(path, __extendRequestCallback(controller_name, action_name, action_definition));
		}
	}

	__extendRequestCallback(controller_name, action_name, action_definition) {
